...
FROM 
	...
	cross join tfwm_nts_securelookups.VehParkLoc_B01ID vl
...
cteVehiclesBase (yearID, countryID, statsregID, laID, vlID, Vehicles_unweighted, Vehicles_weighted)
as
...
as
(select yearID, countryID, statsregID, vlID, sum(Vehicles_unweighted), sum(Vehicles_weighted)
from cteVehiclesBase
group by yearID, countryID, statsregID, vlID
),

cteLaVehicles (yearID, laID, vlID, Vehicles_unweighted, Vehicles_weighted)
as
(select yearID, laID, vlID, sum(Vehicles_unweighted), sum(Vehicles_weighted)
from cteVehiclesBase
group by yearID, laID, vlID
),
